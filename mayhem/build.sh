...
# UPSTREAM's src/lib.rs has #[cfg_attr(test, deny(warnings))]; the current nightly's lint set
# (e.g. mismatched_lifetime_syntaxes) is newer than this old commit, so `cargo test` fails on
# lints that didn't exist when the code was written. --cap-lints=warn downgrades them back to
# warnings without touching any upstream file.
RUSTFLAGS="--cap-lints=warn" cargo test --lib --no-run --no-fail-fast --jobs "$MAYHEM_JOBS"
