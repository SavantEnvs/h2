...
# MUST match build.sh's precompile invocation exactly (RUSTFLAGS="--cap-lints=warn" cargo test
# --lib --no-run --no-fail-fast) so cargo sees the build as current and only RUNS — no recompile
# here.
out="$(RUSTFLAGS="--cap-lints=warn" cargo test --lib --no-fail-fast --jobs "$MAYHEM_JOBS" 2>&1)"
rc=$?
echo "$out"
